...
#### 4.4.1利用future进行函数式编程

**1.函数式编程风格的快速排序**

---

**补充一：什么时候可以进行并发而不影响结果****

​	核心原因：函数式编程 + 数据独立性
​	并发安全性依赖于以下几点设计原则，是的个线程操作独立数据，无需共享或竞争资源。
...
```cpp
auto lower_part = std::move(input);
```

- 操作逻辑：
  - 每次地柜调用前，链表都被分割成两个独立的部分，这两部分数据没有传入传出
  - 而且通过std::move将lowe_part的所有权移交给异步任务，原线程不持有该部分数据。
- 结果正确性保证：
  - 每个线程操作自己独立的数据副本，没有共享数据，自然无需同步锁，避免竞态状态。

2.基准元素的唯一性

```cpp
auto pivot = input.front();
```

- 基准元素在分割时单独切分到result链表中，不会被后续线程修改

**二、函数式编程的无状态特性**

1.递归调用的纯函数行为

- 每个递归调用parallel_quick_sort处理的是当前线程的局部数据副本，不会修改外部数据
- 类似于数学中的分治思想：子问题之间完全独立，但是返回结果又依赖局部计算结果。
...
- splice操作直接移动节点指针，不复制元素，且合并时子链表已完全排序，合并过程无需修改子链表内容

**三、并发任务的分发与同步机制**

- 任务隔离：
  - 异步任务接受的是lower_part的移动副本，与原线程完全分离
- 隐式同步：
  - 通过future.get等待异步任务完成，确保合并结果时子链表已排序完成

**四、何时可以安全改为并发？**

1.数据独立性：任务处理数据是独立的，没有共享，无交叉写入

2.结果合并无冲突：合并结果时无需修改其他线程数据
...
- 因为这个例子中数据完全独立，所以不需要锁或原子操作

2.注意事项：线程数量控制

- 若递归过深，可能产生大量线程，需要依赖线程库优化（如线程池）。
...
定义：
**函数签名**是描述函数的接口特征（名称、参数类型、返回类型）用于唯一标识函数。
**函数指针**是指向函数内存地址的指针变量，用于动态调用函数
...
- 函数签名通过名称 + 参数类型 + 返回类型 唯一标识一个函数， 即使函数名相同，参数了类型不同也会被视为不同的函数

  ```cpp
  void print(int x);//签名：print(int)
  void print(double x);//签名：print(double)）
  ```

**（2）支持模板元编程**

- 在泛型编程中，编译器通过函数签名推导模板参数。
  ...
  - std::result_of<F(A&&)>：根据函数签名推导返回类型。

  - F:可能是函数指针或者函数对象，其签名必须与参数A&&匹配。

总结：

- 函数签名是函数的逻辑标识，用于编译期间类型检查和推导。
- 函数指针是函数的物理入口，用于运行期动态调用。
- 两者在并发编程中缺一不可：签名确保类型安全， 指针实现任务分发。
